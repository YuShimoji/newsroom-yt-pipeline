...
- Autonomous foundation run critical-view readback slice on 2026-06-15: added `newsroom packet list-critical --story <story_id>` to inspect DB-backed critical-view source records without dumping raw URLs. The readback includes article id, source name, source type/role/pool, title, operator note, and recorded timestamp; it does not adopt sources automatically or rebuild packets. Added synthetic tests for note readback and URL suppression. Validation: `.venv\Scripts\python.exe -m pytest tests\test_critical_sources.py -q` -> 8 passed; `.venv\Scripts\python.exe -m pytest -q` -> 108 passed; `git diff --check` -> passed; `.venv\Scripts\python.exe -m newsroom.cli.main packet list-critical --help` -> command available.
...
2. P0: critical-view source path.
   ...
   State: implemented for the active path; `newsroom packet list-critical --story <story_id>` now reads back recorded critical-view sources before packet rebuilds. Do not reselect sources unless runtime artifacts are missing or a new story requires it.
   Owner: operator/editor owns source decisions; assistant owns schema-safe tooling and rebuilds.
   Next move: use `newsroom packet list-critical --story <story_id>` for readback, then preserve or regenerate the active C1/NIST source application, persisted packet, approved materialization, and tightened QuoteManifest only if runtime artifacts are missing.
