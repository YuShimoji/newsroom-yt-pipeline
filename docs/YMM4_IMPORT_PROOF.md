...
- 一次仕様: `<repo_root>\docs\PROJECT_SPEC.md`
- 現在地の正本: `<repo_root>\docs\RUNTIME_STATE.md`
...
```text
<repo_root>\data\exports\episode_<id>
```
...
| speaker が意図通り扱われない | `<episode_dir>\script.csv` / `<repo_root>\configs\speakers.yml` | YMM4 側のキャラクター名と config の対応を確認する |
...
```text
<repo_root>\data\proofs\ymm4_import\<episode_id>\proof.yml
```
...
```text
<repo_root>\docs\templates\ymm4_import_proof_template.yml
```
